...
TxProposal._create.simple = function(txp, opts) {
  ...
  try {
    txp.network = Digicore.Address(txp.toAddress).toObject().network;
  } catch (ex) {}
};
...
TxProposal._create.multiple_outputs = function(txp, opts) {
  ...
  try {
    txp.network = Digicore.Address(txp.outputs[0].toAddress).toObject().network;
  } catch (ex) {}
};
...
TxProposal.prototype.getDigicoreTx = function() {
  var self = this;
  ...
  _.each(sigs, function(x) {
    self._addSignaturesToDigicoreTx(t, x.signatures, x.xpub);
  });
...
TxProposal.prototype.getNetworkName = function() {
  return Digicore.Address(this.changeAddress.address).toObject().network;
};

TxProposal.prototype.getRawTx = function() {
  var t = this.getDigicoreTx();
...
TxProposal.prototype._addSignaturesToDigicoreTx = function(tx, signatures, xpub) {
  var self = this;
  ...
  var i = 0,
    x = new Digicore.HDPublicKey(xpub);

  _.each(signatures, function(signatureHex) {
    var input = self.inputs[i];
    try {
      var signature = Digicore.crypto.Signature.fromString(signatureHex);
      var pub = x.derive(self.inputPaths[i]).publicKey;
      var s = {
        inputIndex: i,
        signature: signature,
        sigtype: Digicore.crypto.Signature.SIGHASH_ALL,
        publicKey: pub,
      };
...
TxProposal.prototype.sign = function(copayerId, signatures, xpub) {
  try {
    ...
    var tx = this.getDigicoreTx();
    this._addSignaturesToDigicoreTx(tx, signatures, xpub);
