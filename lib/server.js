...
/**
 * Creates an instance of the Digicore Wallet Service.
 * @constructor
 */
...
WalletService.prototype.createWallet = function(opts, cb) {
  ...
  try {
    pubKey = new Digicore.PublicKey.fromString(opts.pubKey);
  } catch (ex) {
...
WalletService.prototype.joinWallet = function(opts, cb) {
  ...
  try {
    Digicore.HDPublicKey(opts.xPubKey);
  } catch (ex) {
...
WalletService.prototype._getUtxos = function(addresses, cb) {
  ...
  var networkName = Digicore.Address(addresses[0]).toObject().network;
...
WalletService.prototype._checkTx = function(txp) {
  ...
  try {
    var digicoreTx = txp.getDigicoreTx();
    digicoreError = digicoreTx.getSerializationError(serializationOpts);
    if (!digicoreError) {
      txp.fee = digicoreTx.getFee();
    }
  } catch (ex) {
    log.error('Error building Digicore transaction', ex);
    return ex;
  }

  if (digicoreError instanceof Digicore.errors.Transaction.FeeError)
    return Errors.INSUFFICIENT_FUNDS_FOR_FEE;

  if (digicoreError instanceof Digicore.errors.Transaction.DustOutputs)
    return Errors.DUST_AMOUNT;
  return digicoreError;
};
  ...
  function select(utxos, cb) {
    ...
    _.each(smallInputs, function(input, i) {
      ...
      if (netTotal >= txpAmount) {
        ...
        var smallChangeThreshold = Math.max(Defaults.UTXO_SELECTION_MIN_CHANGE_AMOUNT, Digicore.Transaction.DUST_AMOUNT);
...
WalletService.prototype._validateOutputs = function(opts, wallet) {
  for (var i = 0; i < opts.outputs.length; i++) {
    ...
    var toAddress = {};
    try {
      toAddress = new Digicore.Address(output.toAddress);
    } catch (ex) {
    ...
    if (output.amount < Digicore.Transaction.DUST_AMOUNT) {
      return Errors.DUST_AMOUNT;
    }
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    self.getTx({
      ...
      if (!txp.sign(self.copayerId, opts.signatures, copayer.xPubKey)) {
        ...
        var raw = txp.getDigicoreTx().uncheckedSerialize();
        log.warn('Raw tx:', raw);
...
WalletService.prototype.getTxHistory = function(opts, cb) {
  ...
  self.storage.fetchAddresses(self.walletId, function(err, addresses) {
    ...
    var addressStrs = _.pluck(addresses, 'address');
    var networkName = Digicore.Address(addressStrs[0]).toObject().network;
